...
function formatAnnotationsToBadges(
  ...
  function generateBadge(annotation: AnnotationType) {
    ...
    return (
      <EuiBadge
        key={`annotation_badge_${annotation.feature_name ?? annotation.feature_category}`}
...
export class AnnotationsFieldFormatter extends FieldFormat {
  ...
  static fieldType = [KBN_FIELD_TYPES.STRING, KBN_FIELD_TYPES.UNKNOWN];
  ...
  textConvert: TextContextTypeConvert = (value: string | AnnotationType) => {
    const fieldName = this.param('fieldName');
    if (this.isDebugEnabled())
      this.getLogger()?.debug(
        `[AnnotationsFormat] textConvert: fieldName="${fieldName}", value="${value}"`
      );
  ...
  htmlConvert: HtmlContextTypeConvert = (
    ...
    options: HtmlContextTypeOptions | undefined = {}
  ) => {
    const fieldName = this.param('fieldName');
    if (this.isDebugEnabled())
      this.getLogger()?.debug(
        `[AnnotationsFormat] htmlConvert: fieldName="${fieldName}", value="${value}", options="${JSON.stringify(
          options
        )}"`
      );

    // Find and flatten the TagConfigs for the field specified when the formatter was configured on the Data View
    const flattenedTagConfigs = flattenAnnotationConfigs(
      getTagConfigsForField(fieldName, this.getPluginConfig())
    );

    try {
      if (typeof value === 'string') {
        this.getLogger()?.debug(`[AnnotationsFormat] value is a string`);
        return formatAnnotationsToBadges(
          !['[]', ']'].includes(value) ? JSON.parse(value) : [],
          flattenedTagConfigs
        );
      } else {
        this.getLogger()?.debug(`[AnnotationsFormat] value is an object`);
        return formatAnnotationsToBadges([value], flattenedTagConfigs);
      }
    } catch (e) {
      this.getLogger()?.error(`[AnnotationsFormat] error parsing "${value}" error: ${e}`);
      return ReactDOM.renderToStaticMarkup(<Fragment />);
    }
  };
}
