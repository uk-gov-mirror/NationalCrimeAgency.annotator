...
import {AnnotatorPluginStart} from "../index"
import {
  ANNOTATIONS_ROUTE_PATH, AnnotationType,
  ConfigType,
  UpdateAnnotationHttpResponseOptions, updateAnnotations
} from "../../common";
import {AuthenticatedUser} from '@kbn/security-plugin/server';

async function getCurrentUser(logger: Logger, core: CoreSetup<AnnotatorPluginStart>,
                              request: KibanaRequest): Promise<AuthenticatedUser> {
...
export function defineRoutes(
  logger: Logger, config: ConfigType, core: CoreSetup<AnnotatorPluginStart>, router: IRouter) {
  ...
  router.post(
    ...
    async (context, request, response) => {
      logger.debug(`update: index="${request.params.index}", field="${request.params.field}", id="${request.params.id}", annotation_type="${request.params.annotation_type}"`)
      ...
      if (!annotationConfig) {
        return response.notFound({body: `No matching annotation config was found, field="${request.params.field}"`})
      }
      ...
      if (!currentAnnotations) {
        ...
        await client.asCurrentUser.update({
          index: request.params.index, id: request.params.id, refresh: 'wait_for', script: {
      ...
      logger.info(`action="UPDATED", annotations="${JSON.stringify(annotations)}"`)
